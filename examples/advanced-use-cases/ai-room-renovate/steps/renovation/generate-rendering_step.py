...
config = {
    ...
    "triggers": [
        queue("renovation.render", input=RenderingInput.model_json_schema())
    ],
    "enqueues": [],
...
async def handler(input_data: dict[str, Any], ctx: FlowContext[Any]) -> None:
    ...
    ctx.logger.info("Starting rendering generation with Gemini 2.5 Flash Image", {
    ...
    if not GENAI_AVAILABLE:
        ...
        ctx.logger.error("SDK not available", {"session_id": session_id, "error": error_msg})
        await ctx.state.set(session_id, "renderingError", error_msg)
        return
    ...
    if not roadmap:
        ctx.logger.error("No roadmap found in state", {"session_id": session_id})
        await ctx.state.set(session_id, "renderingError", "No roadmap found")
        return

    ctx.logger.info("Roadmap retrieved", {
        "session_id": session_id,
    ...
    if not rendering_prompt:
        ctx.logger.error("No rendering prompt in roadmap", {
            "session_id": session_id,
        ...
        await ctx.state.set(session_id, "renderingError", "No rendering prompt found in roadmap")
        return

    ctx.logger.info("Rendering prompt found", {
        "session_id": session_id,
    ...
    try:
        ...
        ctx.logger.info("Enhancing prompt with Gemini", {
            "session_id": session_id
        })
        ...
        **Important:** Output your prompt as a single detailed paragraph optimized for photorealistic interior rendering.
        ...
        ctx.logger.info("Prompt enhanced, generating image", {
            "session_id": session_id,
        ...
        ctx.logger.info("Calling Gemini 2.5 Flash Image API", {
            "session_id": session_id,
            "model": model
        })
        ...
        for chunk in client.models.generate_content_stream(
            ...
            if chunk.candidates[0].content.parts[0].inline_data and chunk.candidates[0].content.parts[0].inline_data.data:
                ...
                await ctx.state.set(session_id, "rendering", rendering_data)
                await ctx.state.set(session_id, "renderingCompleted", True)

                ctx.logger.info("Image generated successfully", {
                    "session_id": session_id,
                ...
                if hasattr(chunk.candidates[0].content.parts[0], 'text') and chunk.candidates[0].content.parts[0].text:
                    ...
                    ctx.logger.info("Model text response", {
                        "session_id": session_id,
        ...
        if not image_generated:
            ...
            await ctx.state.set(session_id, "renderingError", error_msg)
            await ctx.state.set(session_id, "renderingNote",
                "The model returned a text response instead of an image. The detailed prompt has been saved.")
            await ctx.state.set(session_id, "renderingPrompt", enhanced_prompt)
