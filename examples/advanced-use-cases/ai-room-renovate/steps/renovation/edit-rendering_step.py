...
config = {
    ...
    "triggers": [queue("renovation.edit", input=EditRenderingInput.model_json_schema())],
    "enqueues": [],
...
async def handler(input_data: dict[str, Any], ctx: FlowContext[Any]) -> None:
    ...
    ctx.logger.info(
        "Starting rendering edit with Gemini 2.5 Flash", {"session_id": session_id, "edit_prompt": edit_prompt}
    )

    # Check if SDK is available
    if not GENAI_AVAILABLE:
        error_msg = "google-genai SDK not installed"
        ctx.logger.error("SDK not available", {"session_id": session_id})
    ...
    if not api_key:
        ...
        ctx.logger.error("API key missing", {"session_id": session_id})
        await ctx.state.set(session_id, "editError", error_msg)
        return

    # Get the original roadmap to get the base rendering prompt
    roadmap = await ctx.state.get(session_id, "roadmap")
    if not roadmap:
        ctx.logger.error("No roadmap found", {"session_id": session_id})
        await ctx.state.set(session_id, "editError", "No roadmap found")
        return
    ...
    if not original_prompt:
        ctx.logger.error("No original prompt found", {"session_id": session_id})
        await ctx.state.set(session_id, "editError", "Cannot edit without original prompt")
        return

    ctx.logger.info("Original prompt retrieved", {"session_id": session_id, "original_length": len(original_prompt)})
...
This is an edited version, ID: edit_{datetime.now().timestamp()}"""
        ...
        ctx.logger.info(
            "Building enhanced edit prompt", {"session_id": session_id, "combined_length": len(combined_prompt)}
        )
        ...
        rewritten_response = client.models.generate_content(model="gemini-2.5-flash", contents=rewrite_prompt)
        enhanced_edit_prompt = rewritten_response.text

        ctx.logger.info(
            "Edit prompt enhanced, generating image",
            {"session_id": session_id, "enhanced_length": len(enhanced_edit_prompt)},
        )
        ...
        ctx.logger.info("Calling Gemini 2.5 Flash Image for edit", {"session_id": session_id, "model": model})
        ...
        for chunk in client.models.generate_content_stream(
            ...
            if (
                ...
                existing_rendering = await ctx.state.get(session_id, "rendering")
                ...
                await ctx.state.set(session_id, "rendering", edited_rendering_data)
                await ctx.state.set(session_id, "renderingCompleted", True)
                ...
                edit_history.append(
                    {"editPrompt": edit_prompt, "version": new_version, "editedAt": datetime.now().isoformat()}
                )
                await ctx.state.set(session_id, "editHistory", edit_history)

                ctx.logger.info(
                    "Rendering edited successfully",
                    {
                ...
                if hasattr(chunk.candidates[0].content.parts[0], "text") and chunk.candidates[0].content.parts[0].text:
                    ...
                    ctx.logger.info(
                        "Model text response during edit", {"session_id": session_id, "response": text_response[:100]}
                    )

        if not image_generated:
            error_msg = "No edited image was generated"
            ctx.logger.error("Edit failed", {"session_id": session_id})
            await ctx.state.set(session_id, "editError", error_msg)
