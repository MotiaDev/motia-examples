...
export const config: EventConfig = {
  ...
  name: 'read-documents',
  flows: ['rag-workflow'],
  subscribes: ['rag.read.documents'],
  emits: [{ topic: 'rag.process.documents', label: 'Start processing documents' }],
  input: InputSchema,
};

export const handler: Handlers['read-documents'] = async (
  input,
  { emit, logger }
) => {
  ...
  logger.info(`Reading documents from folder: ${folderPath}`, { absoluteFolderPath });
  ...
  logger.info(`Found ${supportedFiles.length} supported document files`, {
