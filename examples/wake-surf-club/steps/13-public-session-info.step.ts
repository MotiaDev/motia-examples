...
import { verifyHostToken, getSessionDateRange, getSessionBookings, buildRosterFromBookings, calculateSessionStats } from '../types/utils'

export const config: ApiRouteConfig = {
  type: 'api',
  name: 'SessionInfo',
  description: 'Get session information - public or admin based on authentication',
  method: 'GET',
  path: '/api/sessions',
  responseSchema: {
    200: z.object({
...
export const handler: Handlers['SessionInfo'] = async (req, { emit, logger, state, traceId }) => {
  try {
    ...
    for (const session of allSessions) {
      ...
      const roster = buildRosterFromBookings(confirmedBookings, allFriends, 'confirmed')
      ...
      if (isAdmin && waitlistedBookings.length > 0) {
        sessionData.waitlist = buildRosterFromBookings(waitlistedBookings, allFriends, 'waitlisted').map((rosterItem, index) => ({
    ...
    logger.error('Failed to get sessions info', { error: error.message, traceId })
