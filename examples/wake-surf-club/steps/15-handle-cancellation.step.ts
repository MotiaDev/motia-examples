...
export const config: EventConfig = {
  ...
  description: 'Handle booking cancellation and waitlist promotion',
...
export const handler: Handlers['HandleCancellation'] = async (input, { emit, logger, state, traceId }: any) => {
  ...
  try {
    ...
    if (confirmedBookings.length < session.capacity && waitlistedBookings.length > 0) {
      ...
      if (friend) {
        ...
        if (secret) {
          const cancelToken = createSignedToken(sessionId, friend.phoneE164, secret)
