...
import { z } from 'zod'
import { verifyHostToken, createSignedToken, calculateNextTuesdayDate, getActiveFriends } from '../types/utils'
...
export const handler: Handlers['AdminSendInvites'] = async (req, { emit, logger, state, traceId }) => {
  try {
    ...
    logger.error('Failed to send manual invite blast', { error: error.message, traceId })
