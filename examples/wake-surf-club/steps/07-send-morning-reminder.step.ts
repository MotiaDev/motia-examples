...
export const handler: Handlers['SendMorningReminder'] = async ({ emit, logger, state, traceId }) => {
  try {
    // Get today's session
    ...
    const confirmedBookings = allBookings.filter(
      (booking: any) => booking.sessionId === session.id && booking.status === 'confirmed'
    ) as Booking[]
    
    if (confirmedBookings.length === 0) {
      logger.info('No confirmed bookings for today\'s session', { sessionId: session.id, traceId })
      return
    }
    
    // Build roster list
    const rosterNames = confirmedBookings.map((booking: Booking) => {
      const friend = allFriends[booking.friendId] as Friend
      return friend?.name || 'Friend'
    })
    
    const rosterText = rosterNames.length > 0 ? rosterNames.join(', ') : 'Just you!'
    ...
    for (const booking of confirmedBookings) {
      try {
        const friend = allFriends[booking.friendId] as Friend
        if (!friend) {
          logger.warn('Friend not found for booking', { bookingId: booking.id, friendId: booking.friendId, traceId })
          continue
        }
    ...
    logger.error('Morning reminder blast failed', { error: error.message, traceId })
