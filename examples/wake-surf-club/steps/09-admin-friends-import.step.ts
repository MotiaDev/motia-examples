...
import { verifyHostToken, normalizePhone, isValidPhoneE164 } from '../types/utils'
...
export const handler: Handlers['AdminImportFriends'] = async (req, { emit, logger, state, traceId }) => {
  try {
    ...
    for (const friendData of friends) {
      try {
        ...
        const existingFriend = await state.get('friends', normalizedPhone) as Friend
        if (existingFriend) {
