...
export const handler: Handlers['DirectBooking'] = async (req, { emit, logger, state, traceId }) => {
  try {
    const { sessionId, friendName, phoneE164 } = req.body
    
    logger.info('Processing direct booking request', { sessionId, friendName, phoneE164, traceId })
    ...
    const existingBooking = await state.get('bookings', bookingKey) as Booking
