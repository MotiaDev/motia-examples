...
export const config: EventConfig = {
  ...
  input: z.object({
    to: z.string(),
    body: z.string(),
    type: z.enum(['invite', 'confirmation', 'reminder', 'promotion', 'cancellation']),
    dedupeKey: z.string().optional()
  }),
  flows: ['wake-surf-club']
}

export const handler: Handlers['SendSms'] = async (input, { emit, logger, state, traceId }) => {
  ...
  try {
    ...
    if (dedupeKey) {
      ...
      if (existing) {
        logger.info('SMS already sent, skipping duplicate', { dedupeKey, to, type, traceId })
        return
      }
    }
