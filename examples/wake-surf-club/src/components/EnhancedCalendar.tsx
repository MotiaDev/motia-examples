...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Clock, Users, MapPin, Plus, Edit, Trash2, X } from 'lucide-react'
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, addMonths, subMonths, isToday, isPast, parseISO, startOfWeek, endOfWeek, isBefore } from 'date-fns'
...
const SessionModal: React.FC<SessionModalProps> = ({
  ...
  const canBook = !isSessionFull && !isSessionPast && session.status === 'published'
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <p>Available: {Math.max(0, session.capacity - bookingsList.length)} spots</p>
                </div>
              </div>
              ...
              {mode === 'admin' && (
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    className="w-full py-2 px-4 bg-red-600 text-white rounded-lg hover:bg-red-700 flex items-center justify-center space-x-2"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {bookingsList.map((booking) => (
                  <div key={booking.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">{booking.friendName}</p>
                      <p className="text-sm text-gray-500">{booking.phoneMasked}</p>
                    </div>
...
const EnhancedCalendar: React.FC<EnhancedCalendarProps> = ({
  ...
  const handleCancelBooking = async (bookingId: string) => {
    if (confirm('Are you sure you want to cancel this booking?')) {
      ...
      if (selectedSession) {
        const session = sessions.find(s => s.session.id === selectedSession.id)
        if (session) {
          setSelectedBookings(session.bookings || [])
        }
      }
              ...
              return (
                ...
                >
                  <div className="flex flex-col h-full">
                    ...
                    {session && (
                      <div 
                        className="flex-1 flex flex-col justify-center cursor-pointer hover:bg-gray-50 rounded p-1"
                        onClick={(e) => {
                          e.stopPropagation()
                          handleDayClick(day)
                        }}
                    ...
                    {mode === 'admin' && !session && isCurrentMonth && (
                      <div className="flex-1 flex items-center justify-center">
                        <div className="w-2 h-2 bg-gray-300 rounded-full hover:bg-blue-400 transition-colors"></div>
                      </div>
                    )}
          ...
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-6">
              ...
              <p className="text-lg font-medium text-gray-900">
                {createDate && format(parseISO(createDate), 'EEEE, MMMM d, yyyy')}
              </p>
